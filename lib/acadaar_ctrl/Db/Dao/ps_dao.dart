...
import 'package:zlga/acadaar_ctrl/Db/Dao/ps_app_database.dart';
import 'package:zlga/acadaar_ctrl/ObjectHolder/ps_map_object.dart';
...
abstract class DrDao<T extends DrObject<T>> {
  // dynamic dao;
  ...
  Future<Database> get db async => await DrAppDatabase.instance.database;
  ...
  Future<DrResource<List<T>>> getByKey(String key, String value,
      {List<SortOrder> sortOrderList,
      DrStatus status = DrStatus.SUCCESS}) async {
    ...
    return DrResource<List<T>>(status, '', resultList);
  }

  Future<dynamic> getAllWithSubscription(
      {StreamController<DrResource<List<T>>> stream,
      Finder finder,
      DrStatus status = DrStatus.SUCCESS,
  ...
  Future<DrResource<List<T>>> getAll(
      {Finder finder, DrStatus status = DrStatus.SUCCESS}) async {
    finder ??= Finder();
    final dynamic recordSnapshots = await dao.find(
      await db,
      finder: finder,
  ...
  Future<DrResource<T>> getOne(
    ...
    return DrResource<T>(status, '', result);
  }

  Future<DrResource<List<T>>> getAllByJoin<K extends PsMapObject<dynamic>>(
      String primaryKey, DrDao<DrObject<dynamic>> mapDao, dynamic mapObj,
      {List<SortOrder> sortOrderList,
      DrStatus status = DrStatus.SUCCESS}) async {
    final DrResource<List<DrObject<dynamic>>> dataList = await mapDao.getAll(
  ...
  Future<DrResource<List<T>>> getAllByMap<K extends PsMapObject<dynamic>>(
      String primaryKey,
      String mapKey,
      String paramKey,
      DrDao<DrObject<dynamic>> mapDao,
